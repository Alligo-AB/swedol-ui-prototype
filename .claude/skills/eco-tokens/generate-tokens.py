...
def parse_colors(dist):
    ...
    pattern = re.compile(
        r'(?:/\*(?P<comment>.*?)\*/\s*)?--color-(?P<name>[a-z0-9-]+):\s*(?P<value>[^;]+);',
        re.DOTALL
    )
    colors = {}
    for m in pattern.finditer(css):
        ...
        comment = re.sub(r'\s+', ' ', (m.group("comment") or "")).strip()
        if len(comment) > 160:
            comment = comment[:157] + "..."
...
def parse_spacing(dist):
    ...
    spacing = {}
    for m in re.finditer(r'(--dimension-spacing-space-(\d+)):\s*([^;]+);', css):
        full_var, n, val = m.group(1), m.group(2), m.group(3).strip()
        rem_match = re.search(r'([\d.]+rem)', val)
        spacing[f"space-{n}"] = {"var": full_var, "px": f"{n}px", "value": rem_match.group(1) if rem_match else val}
    return dict(sorted(spacing.items(), key=lambda kv: int(kv[0].split('-')[1])))


def parse_radius_border(dist):
    dim = read(dist, "css/base/dimension.css")
    radius = {}
    for m in re.finditer(r'(--dimension-radius-([a-z0-9-]+)):\s*([^;]+);', dim):
        full_var, name, val = m.group(1), m.group(2), m.group(3).strip()
        rem_match = re.search(r'([\d.]+rem)', val)
        radius[name] = {"var": full_var, "value": rem_match.group(1) if rem_match else val}

    border_css = read(dist, "css/base/border.css")
    borders = {}
    for m in re.finditer(r'(--border-([a-z0-9-]+)):\s*([^;]+);', border_css):
        full_var, name, val = m.group(1), m.group(2), m.group(3).strip()
        borders[name] = {"var": full_var, "value": val}
...
def parse_shadows(dist):
    ...
    pattern = re.compile(
        r'(?:/\*(?P<comment>.*?)\*/\s*)?(?P<var>--shadow-[a-z0-9-]+):\s*(?P<value>[^;]+);',
        re.DOTALL
    )
    shadows = {}
    for m in pattern.finditer(css):
        full_var = m.group("var")
        name = full_var.replace("--shadow-", "")
        comment = re.sub(r'\s+', ' ', (m.group("comment") or "")).strip()
        shadows[name] = {"var": full_var, "value": m.group("value").strip(), "description": comment or None}
    return shadows
